...
//        List menuList = getSession().createQuery("select * from SysMenus as m where m.sys=true and m.deep=1").list();
        List menuList = getSession().createQuery("select distinct " +
                ...
                " from SysMenus m left join m.resources as resources where m.deep=1 and m.roleId is null order by menuIndex").setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
        ...
        if(pager != null && pager.getParams()!=null && pager.getParams().get("parentid") != null){
           String parentid = (String) pager.getParams().get("parentid");
            List menuList = getSession().createQuery("select distinct " +
                    ...
                    "m.children.size as childrenSize," +
                    "case  when m.children.size>0 then 'closed' else 'open' end as state " +
                    " from SysMenus m left join m.resources as resources where m.parent.id=:parentid order by menuIndex").setString("parentid",parentid).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
    ...
    public Boolean add(SysMenus menus) {
        ...
        if(menus.getResources() != null && StringUtil.isNotEmpty(menus.getResources().getId())){
            resources = (SysResources) getSession().get(SysResources.class,menus.getResources().getId());
            if(resources == null){
                return false;
            }
        }else{
            menus.setResources(null);
        }
        if(StringUtil.isNotEmpty(menus.getId())){
            SysMenus menus1 = (SysMenus) getSession().get(SysMenus.class,menus.getId());
            if(menus1 != null){
               if(resources != null){
                   menus1.setResources(resources);
               }
