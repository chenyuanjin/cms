...
function modifyPassword() {
    ...
    if(repassword1 == newpassword1 ) {
        if(repassword1.length<17 && repassword1.length>5) {
            //alert(oldpassword1);
            $.ajax({
                url: "admin/user/modifypassword",
                dataType: 'json',
                type: 'POST',
                data: {oldpassword: oldpassword1, newpassword: repassword1},
                success: function (data) {

                    if (data.success) {
                        $("#change_user_password_form").dialog("close");
                        $.messager.confirm('提示', '操作成功!请退出重新登录!！', function (r) {
                            if (r) {
            ...
            showMessager("密码长度6-16位！");
        }
    }else{
        showMessager("两次输入不一致！");
    }
}
