...
public class RoleMenuDaoImpl extends BaseDaoImpl<SysRoleMenus,String> implements RoleMenuDao {
	...
	private void createMenu(Collection<SysMenus> menusList, SysRoles roles, Collection<String> ids){
		...
		while (it.hasNext()){
			...
			Collection<SysMenus> li = getSession().createQuery("select distinct m from SysMenus m left join m.children ms where (m.deep=:deep and m.resources is null and ms.resources.id in (:ids) and m.parent.id=:pid) or (m.deep=:deep and m.resources is not null and m.resources.id in (:ids) and m.parent.id=:pid) order by m.menuIndex")
	...
	public ListResult<SysRoleMenus> loadRoleMenu(String roleId, String parentId) {
		...
		if(roleMenus != null){
			result.setDataList(getSession().createQuery("select distinct " +
					...
					"rm.ind as index " +
					",case when (select count(*) from SysRoleMenus rms left join rms.menu ms where rm.role.id=:roleId and ms.parent.id=m.id)>0 then 'closed' else 'open' end  as state" +//�ж��Ƿ����ӽڵ�
					" from SysRoleMenus rm left join rm.menu m left join m.resources r where rm.role.id=:roleId and m.parent.id=:parentId order by rm.ind")
...
//	        getSession().setCacheMode(CacheMode.REFRESH);
		result.setDataList(getSession().createQuery("select distinct " +
				...
				"r.resourcePath as url" +
				",case when (select count(*) from SysRoleMenus rms left join rms.menu ms where rm.role.id=:roleId and ms.parent.id=m.id)>0 then 'closed' else 'open' end  as state" +//�ж��Ƿ����ӽڵ�
				" from SysRoleMenus rm left join rm.menu m left join m.resources r where rm.role.id=:roleId and m.deep=1 order by rm.ind")
				.setString("roleId", roleId)
				.setCacheable(true)
				.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP)
				.list());
	...
	public Boolean createRoleMenu(String roleId) {
		...
		if(roles != null ){
			List menus = getSession().createQuery("select distinct m from SysMenus m left join m.children ms where m.deep=1 and ((m.resources.id in (:ids) and m.resources is not null) or (ms.resources.id in (:ids) and m.resources is null)) order by m.menuIndex").setParameterList("ids",ids).list();
		...
		if(obj != null && obj instanceof SysUsers){
			...
			if(ids.size()>0){
				result.setDataList(getSession().createQuery("select distinct " +
						...
						"rm.ind as index, " +
						"r.resourcePath as url" +
						",case when (select count(*) from SysRoleMenus rms left join rms.menu ms where rm.role.id= rm.role.id and ms.parent.id=m.id)>0 then 'closed' else 'open' end  as state" +//�ж��Ƿ����ӽڵ�
						" from SysRoleMenus rm left join rm.menu m left join m.resources r where rm.role.id in (:roleId) and m.deep=1 order by rm.ind")
		...
		if(obj != null && obj instanceof SysUsers){
			...
			if(ids.size()>0){
				result.setDataList(getSession().createQuery("select distinct " +
						...
						"r.resourcePath as url" +
						",case when (select count(*) from SysRoleMenus rms left join rms.menu ms where rm.role.id= rm.role.id and ms.parent.id=m.id)>0 then 'closed' else 'open' end  as state" +//�ж��Ƿ����ӽڵ�
						" from SysRoleMenus rm left join rm.menu m left join m.resources r where rm.role.id in (:roleId)  and m.parent.id=:parentId order by rm.ind")
						.setParameterList("roleId", ids)
	...
	public Boolean moveUp(String roleid, String menuid) {
		...
		if(roleMenus != null && roleMenus.getInd()>1){
			...
			if(roleMenus.getMenu().getParent() != null){
				SysRoleMenus = (com.authority.model.SysRoleMenus) getSession().createQuery("from SysRoleMenus m where m.role.id=:roleid and m.menu.parent.id=:parentid and m.ind<:index order by m.ind desc ")
						.setInteger("index",index)
						.setString("roleid",roleid)
						.setMaxResults(1)
						.setString("parentid",roleMenus.getMenu().getParent().getId()).uniqueResult();
			}else{
				SysRoleMenus = (com.authority.model.SysRoleMenus) getSession().createQuery("from SysRoleMenus m where m.role.id=:roleid and m.menu.parent is null and m.ind<:index order by m.ind desc ")
						.setInteger("index",index)
						.setString("roleid",roleid)
						.setMaxResults(1)
						.uniqueResult();
	...
	public Boolean moveDown(String roleid, String menuid) {
		...
		if(roleMenus != null && roleMenus.getInd()>=1){
			...
			if(roleMenus.getMenu().getParent() != null){
				SysRoleMenus = (com.authority.model.SysRoleMenus) getSession().createQuery("from SysRoleMenus m where m.role.id=:roleid and m.menu.parent.id=:parentid and m.ind>:index order by m.ind asc ")
						.setInteger("index",index)
						.setString("roleid",roleid)
						.setMaxResults(1)
						.setString("parentid",roleMenus.getMenu().getParent().getId()).uniqueResult();
			}else{
				SysRoleMenus = (com.authority.model.SysRoleMenus)getSession().createQuery("from SysRoleMenus m where m.role.id=:roleid and m.menu.parent is null and m.ind>:index order by m.ind asc ")
						.setInteger("index",index)
						.setString("roleid",roleid)
						.setMaxResults(1)
						.uniqueResult();
