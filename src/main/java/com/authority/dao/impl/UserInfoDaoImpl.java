...
public class UserInfoDaoImpl implements UserInfoDao {
	...
	public SysUsers getUserByName(String username) {
		try{
			...
			if(users!=null){
				users.setAuthorities(this.grantedAuthorityCollection(users));
				session.evict(users);
				session.close();
				return users;
			}
	...
	private Collection<GrantedAuthority> grantedAuthorityCollection(
			SysUsers users) {
		Set<GrantedAuthority> authoritiesList = new HashSet<GrantedAuthority>();
		if(users!=null){
			...
			if(it.hasNext()){
				...
				if(o instanceof SysRoles){
					...
					authoritiesList.addAll(mSysRoles.getRole_authorities());
				}
			}
		}
